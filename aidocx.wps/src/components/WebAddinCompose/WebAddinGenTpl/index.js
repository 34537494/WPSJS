...
import { UploadDocContent,updateStatus } from "../../WebAddinCommon/UploadDocContent";
...
const checktip = [
  ...
  ["对源文档中有特异字形或者缩进对部分进行统计分析，归纳出新规则，建立样式去规范类似段落", "智能生成新规则"],
  ["基于一般性规则，对样式的细节，进行微调，达到美化对效果，比如启用孤行控制", "美化优化"],
...
const optionarr = [
  [
    ...
    { value: "2", text: "内置标题", tip: "按照已经设置好的标题 1、标题2……转换" },
  ...
  [
    {
      ...
      text: "最小值模式",
      tip:
        "最小值模式不会出现文字重叠情况，段落内部同时有图片和文字时，图片不会被掩盖，但是调整字体大小时，可能根据最小值设置情况自动改变行距"
    },
    {
      value: "2",
      text: "固定值模式",
      tip:
        "容易被用户理解，采用固定值模式后，在使用样式数量多对情况下，编辑过程中样式往往自动增加，为样式使用带来麻烦，段落内部同时有图片和文字时，图片往往会被掩盖"
    },
    {
      value: "3",
      text: "倍距模式",
      tip:
        "采用倍距模式，可以防止编辑过程中样式自动增加，段落内部同时有图片和文字时，图片不会被掩盖，调整页面行距可以统一调整所有间距"
    },
    { value: "4", text: "智能模式", tip: "依据对现有文档对统计分析，决定是用那种模式之一" }
  ],
  [
    { value: "1", text: "倍距模式", tip: "需调整页面行距可以一次性调整所有间距" },
    { value: "2", text: "固定值模式", tip: "容易被用户理解，不能一次性调整所有间距" },
    { value: "3", text: "智能模式", tip: "依据对现有文档对统计分析，决定是用倍距模式还是固定值模式" }
  ],
  [
    { value: "1", text: "极不规范", tip: "极度不规范的文档往往来自于各种途径的抄抄写写" },
    {
      value: "2",
      text: "很不规范",
      tip: "很不规范的文档往往来自不同人共同写的文档，或者文档从不同的Word文档复制粘贴得到的"
    },
    { value: "3", text: "较不规范", tip: "较不规范往往来自于撰写文档过程中使用文档编辑软件不地道规不规范" },
    {
      value: "4",
      text: "比较规范",
  ...
  [
    { value: "1", text: "很不规范", tip: "往往来自于OCR识别、PDF转换、网页复制等得到的文档" },
    {
      value: "2",
      text: "较不规范",
    ...
    { value: "4", text: "已很规范", tip: "正确使用文档编辑软件并手工校对过的文档" }
  ]
];
const demooptList = [
  [
...
class WebAddinGenTpl extends React.Component {
  ...
  componentDidUpdate() {
    console.log("genTpl_componentDidUpdate_this.props.location:", this.props.location);
    updateStatus("");
  }
  ...
  handleGenTpl = (state, showSignUp, e) => {
    ...
    if (
      ...
      if (JSON.stringify(SvrData) !== JSON.stringify(NewData)) {
        const curdata = {};
        curdata.newset = NewData.ckserial.join(",") + "_" + NewData.opserial.join(",") + "_" + NewData.demoopt.join(",");
        curdata.user_id = this.state.curUser.user_id;
        curdata.type = "3";
        handlePost(`${apiPublicPath}users/setUserWebOpt`, curdata).then(result => {
      ...
      UploadDocContent(`${apiPublicPath}tools/composeFilesRecord`, "1",state.curUser.user_id, this.autoGenTpl.bind(this));
    }
  };
  ...
  render() {
    ...
    const optionset = (
      <div>
        <div className="checkTop">
          ...
          <Stack styles={stackStyles} tokens={stackTokens}>
            <Stack.Item align="center" verticalAlignment="center" styles={stackItemStyles}>
              <div style={{ display: "inline-block", alignItems: "center", paddingRight: 12 }}>类型：</div>
        ...
        <div className="checkOpt" hidden={hideopt}>
          <Stack styles={stackStyles} tokens={stackTokens}>
            <Stack.Item align="left" styles={stackItemStyles}>
              <div style={{ display: "inline-block" }}>{this.genCheckOpt(checktip[checkIndex], checkIndex++)}</div>
              <div style={{ display: "inline-block" }}>
                {this.genOptionOpt(optiontip[OptionIndex], optionarr[OptionIndex], OptionIndex++)}
              </div>
            </Stack.Item>
        ...
        <TplDetailsList history={history} curUser={this.state.curUser} listRenew={this.state.listRenew}/>
      </div>
    );
    ...
    return (
      ...
      {({ state, showSignUp }) => (
          ...
          <PrimaryButton primary styles={ButtonStyles} onClick={this.handleGenTpl.bind(this, state, showSignUp)}>
            生成新排版方案
          </PrimaryButton>
          {optionset}
        </div>
